...
def upload_to_s3(file_path, bucket_path=None):
    try:
        ...
        s3_client.upload_file(file_path, bucket_name, s3_file_name, ExtraArgs={'ACL': 'public-read'})
...
def upload_obj_to_s3(file, file_name, bucket_path=None):
    try:
        ...
        s3_client.upload_fileobj(file, bucket, bucket_path + '/' + file_name, ExtraArgs={'ACL': 'public-read'})
        logger.info("Upload Successful")
        return True
    except FileNotFoundError:
        logger.info("The file was not found")
...
def download_from_s3(s3_file_name, output_path=None):
    ...
    file_path = os.getcwd() + f"/tmp/output/{file_name}" if output_path is None else output_path
